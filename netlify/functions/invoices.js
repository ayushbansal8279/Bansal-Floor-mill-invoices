...
export const handler = async (event, context) => {
  ...
  try {
    ...
    if (event.httpMethod === "GET" && path === "") {
      const invoices = await Invoice.find({}).sort({ savedAt: -1 }).lean();
    ...
    if (event.httpMethod === "GET" && isLastNumber) {
      const doc = await LastInvoiceNumber.findById("lastInvoiceNumber").lean();
    ...
    // =========================================
    if (event.httpMethod === "GET" && path) {
      const invoice = await Invoice.findOne({ invoiceNumber: path }).lean();
