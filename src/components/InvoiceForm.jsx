...
import { saveCompany, getCompanies } from "../utils/companyStorage";
...
const InvoiceForm = ({
  ...
  const handleToNameChange = (value) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (invoiceInfo.toName.trim()) {
        await saveCompany({
          name: invoiceInfo.toName,
          nameHindi: invoiceInfo.toNameHindi,
          address: invoiceInfo.toAddress,
        });
      }
      ...
      if (isEditMode && invoiceInfo.invoiceNumber) {
        ...
        if (success) {
          ...
          const errorMsg =
            "Failed to update invoice. Please check if the server is running.";
        ...
        try {
          ...
          if (success) {
            ...
            const errorMsg =
              "Failed to save invoice. Please check your connection and try again.";
            setSubmitError(errorMsg);
            toast.error(errorMsg);
          }
        } catch (error) {
      ...
      <div className="form-section">
        ...
        <div className="form-group">
          ...
          <input
            type="text"
            value={
              companyNameLang === "hindi"
                ? invoiceInfo.fromName
                : invoiceInfo.fromNameEng
            }
            onChange={(e) => {
        ...
        <div className="form-group">
          ...
          <textarea
            ...
            onChange={(e) =>
              setInvoiceInfo({ ...invoiceInfo, fromAddress: e.target.value })
            }
            rows="3"
          />
        </div>
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) =>
              setInvoiceInfo({ ...invoiceInfo, fromEmail: e.target.value })
            }
          />
        </div>
      </div>
  ...
  if (newLang === 'hindi') {
    setInvoiceInfo(prev => ({
      ...prev,
      toNameHindi: prev.toNameHindi || prev.toName // only fallback
    }))
  } else {
    setInvoiceInfo(prev => ({
      ...prev,
      toName: prev.toName || prev.toNameHindi // fallback
    }))
  }
  ...
  const filtered = companies.filter(c =>
    (c.nameHindi && c.nameHindi.includes(value)) ||
    c.name.toLowerCase().includes(value.toLowerCase())
  )
  ...
  setShowSuggestions(true)
        ...
        <div className="form-group">
          ...
          <textarea
            ...
            onChange={(e) =>
              setInvoiceInfo({ ...invoiceInfo, toAddress: e.target.value })
            }
            rows="3"
          />
        </div>
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) =>
              setInvoiceInfo({ ...invoiceInfo, toVehicle: e.target.value })
            }
              ...
              <div className="col-item">
                ...
                >
                  ...
                  {predefinedItems.map((preItem) => (
                    <option key={preItem.id} value={preItem.name}>
                      {preItem.name}
                    </option>
                  ))}
                </select>
