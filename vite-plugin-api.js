...
const LAST_INVOICE_NUMBER_FILE = path.join(
  __dirname,
  "src",
  "data",
  "lastInvoiceNumber.json",
);
...
if (!fs.existsSync(LAST_INVOICE_NUMBER_FILE)) {
  fs.writeFileSync(
    LAST_INVOICE_NUMBER_FILE,
    JSON.stringify({ lastNumber: 0 }, null, 2),
  );
}
...
const predefinedItemsPath = path.join(
  __dirname,
  "src",
  "data",
  "predefinedItems.json",
);
if (fs.existsSync(predefinedItemsPath)) {
  try {
    ...
    if (!fs.existsSync(ITEMS_FILE)) {
      ...
      console.log(
        `Loaded ${itemsWithIds.length} items from predefinedItems.json`,
      );
    } else {
      ...
      if (existingItems.length === 0 && items.length > 0) {
        ...
        fs.writeFileSync(ITEMS_FILE, JSON.stringify(itemsWithIds, null, 2));
        console.log(
          `Loaded ${itemsWithIds.length} items from predefinedItems.json (file was empty)`,
        );
      }
    }
  } catch (error) {
...
const setLastInvoiceNumber = (number) => {
  try {
    fs.writeFileSync(
      LAST_INVOICE_NUMBER_FILE,
      JSON.stringify({ lastNumber: number }, null, 2),
      "utf8",
    );
    return true;
  } catch (error) {
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (req.method === "GET" && isBasePath) {
          ...
          try {
            ...
            res.statusCode = 500;
            res.end(
              JSON.stringify({ error: "Failed to fetch last invoice number" }),
            );
          }
          ...
          try {
            ...
            res.statusCode = 500;
            res.end(
              JSON.stringify({ error: "Failed to fetch next invoice number" }),
            );
          }
          ...
          try {
            ...
            res.setHeader(
              "Content-Disposition",
              `attachment; filename=invoices_${new Date().toISOString().split("T")[0]}.json`,
            );
            res.end(JSON.stringify(invoices));
          } catch (error) {
          ...
          try {
            ...
            const invoiceNumber =
              url.split("/").pop() ||
              url.replace(/^\/api\/invoices\//, "").replace(/^\//, "");
          ...
          req.on("end", async () => {
            try {
              ...
              const invoiceNum = extractInvoiceNumber(
              ...
              res.end(
                JSON.stringify({ success: true, invoice: invoice.toObject() }),
              );
            } catch (error) {
          ...
          req.on("end", async () => {
            try {
              ...
              updatedData.savedAt =
                existingInvoice.savedAt || new Date().toISOString();
        ...
        if (req.method === "GET") {
          ...
          req.on("end", async () => {
            try {
              ...
              await newItem.save();
              res.end(
                JSON.stringify({ success: true, item: newItem.toObject() }),
              );
            } catch (error) {
          ...
          req.on("end", async () => {
            try {
              ...
              res.end(
                JSON.stringify({ success: true, item: updatedItem.toObject() }),
              );
            } catch (error) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        const urlParts = req.url.split("/").filter((p) => p);
        const companyId = urlParts.length >= 3 ? urlParts[2] : null;
        res.setHeader("Content-Type", "application/json");
        res.setHeader("Access-Control-Allow-Origin", "*");
        ...
        if (req.method === "GET" && req.url.includes("/suggestions")) {
          ...
          req.on("end", async () => {
            try {
              ...
              const nameHindi =
                typeof parsed.nameHindi === "string" ? parsed.nameHindi : "";
              const address =
              ...
              res.end(
                JSON.stringify({
                  ...
                  nameHindi: newCompany.nameHindi,
          ...
          try {
            ...
            res.statusCode = 500;
            res.end(JSON.stringify({ error: "Failed to delete company" }));
          }
        } else if (req.method === "PUT" && companyId) {
          let body = "";
          ...
          req.on("end", async () => {
            try {
              ...
              res.statusCode = 500;
              res.end(JSON.stringify({ error: "Failed to update company" }));
            }
          });
        } else {
          next();
